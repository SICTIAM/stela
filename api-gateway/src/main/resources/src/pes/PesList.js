...
class PesList extends Component {
  ...
  sort = clickedColumn => {
    ...
    if (column !== clickedColumn) {
      this.setState({ column: clickedColumn, direction: 'ASC' }, () =>
        this.submitForm()
      );
      return;
    }
    this.setState({ direction: direction === 'ASC' ? 'DESC' : 'ASC' }, () =>
      this.submitForm()
    );
  };
  ...
  render() {
    ...
    const statusDisplay = histories => {
      ...
      return (
        <span>
          {moment(lastHistory.date).format('DD/MM/YYYY')} :{' '}
          {t(`pes.status.${lastHistory.status}`)}
        </span>
      );
    };
    ...
    const metaData = [
      { property: 'uuid', displayed: false, searchable: false },
      {
        property: 'creation',
        displayed: true,
        displayName: t('pes.fields.creation'),
        searchable: true,
        displayComponent: creationDisplay
      },
      {
        property: 'objet',
        displayed: true,
        displayName: t('pes.fields.objet'),
        searchable: true
      },
      {
        property: 'comment',
        displayed: true,
        displayName: t('pes.fields.comment'),
        searchable: true
      },
      {
        property: 'pesHistories',
        displayed: true,
        displayName: t('pes.fields.status'),
        searchable: false,
              ...
              <Form onSubmit={this.submitForm}>
                <FormFieldInline htmlFor="objet" label={t('pes.fields.objet')}>
                  <input
                    ...
                    value={search.objet}
                    onChange={e =>
                      this.handleFieldChange('objet', e.target.value)
                    }
                  />
                </FormFieldInline>
                <FormFieldInline
                  htmlFor="creationFrom"
                  label={t('pes.fields.creation')}
                >
                  <Form.Group style={{ marginBottom: 0 }} widths="equal">
                    <FormField
                      htmlFor="creationFrom"
                      label={t('api-gateway:form.from')}
                    >
                      <InputDatetime
                        id="creationFrom"
                        timeFormat={false}
                        ...
                        onChange={date =>
                          this.handleFieldChange('creationFrom', date)
                        }
                      />
                    </FormField>
                    <FormField
                      htmlFor="creationTo"
                      label={t('api-gateway:form.to')}
                    >
                      <InputDatetime
                        id="creationTo"
                        timeFormat={false}
                        ...
                        onChange={date =>
                          this.handleFieldChange('creationTo', date)
                        }
                      />
                    </FormField>
                  </Form.Group>
