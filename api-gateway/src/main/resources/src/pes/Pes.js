...
class Pes extends Component {
      ...
      fetchWithAuthzHandling({ url: '/api/pes/' + this.props.uuid })
        ...
        .catch(response => {
          this.setState({
            fetchStatus:
              response.status === 404
                ? 'pes.page.non_existing_pes'
                : 'api-gateway:error.default'
          });
  ...
  getAgentInfos = () => {
    fetchWithAuthzHandling({
      url: '/api/admin/profile/' + this.state.pes.profileUuid
    })
      .then(response => response.json())
      .then(json =>
        this.setState({
          agent: `${json.agent.given_name} ${json.agent.family_name}`
        })
      );
  };
  ...
  reSendFlux = () => {
    fetchWithAuthzHandling({
      url: '/api/pes/resend/' + this.props.uuid,
    ...
    const canResend =
      ...
      (lastHistory.status === 'MAX_RETRY_REACH' ||
        ((lastHistory.status === 'SENT' ||
          lastHistory.status === 'RESENT' ||
          lastHistory.status === 'MANUAL_RESENT') &&
    ...
    return (
      <Page title={pes.objet}>
        <LoadingContent fetchStatus={this.state.fetchStatus}>
          <Anomaly
            header={t('pes.page.title_anomaly')}
          ...
          <Segment>
            <Label
              ...
              color={
                lastHistory ? this.getStatusColor(lastHistory.status) : 'blue'
              }
              ribbon
            >
            ...
            <div style={{ textAlign: 'right' }}>
              {canResend && (
                <ConfirmModal
                  onConfirm={this.reSendFlux}
                  text={t('pes.page.re_send_confirm')}
                >
            ...
            <Field htmlFor="attachment" label={t('pes.fields.attachment')}>
              <FieldValue id="attachment">
                <LinkFile
                  url={`/api/pes/${pes.uuid}/file`}
                  text={pes.attachment.filename}
                />
              </FieldValue>
            </Field>
          </Segment>
