...
const InputTextControlled = (
  { component: Component, onChange, value, maxLength, ...props },
  { t }
) => (
  <div>
    <Component
      value={value}
      onChange={(e, { id, value }) =>
        value.length <= maxLength && onChange(id, value)
      }
      {...props}
    />
    <p style={{ fontStyle: 'italic' }}>
      {t('api-gateway:form.max_string_length', {
        length: maxLength,
        remaining: maxLength - (value ? value.length : 0)
      })}
    </p>
  </div>
);
...
const Tooltip = ({ icon, text }) => (
  <i
    className={`tooltip ${icon} small circular icon`}
...
const MigrationSteps = (
  { icon, title, description, status, onClick, reset, disabled = false },
  { t }
) => (
  <Step.Group fluid>
    <Step
    ...
    {status === 'DONE' && (
      <Step
        disabled={disabled}
        style={{ color: '#21ba45', background: '#f3f4f5' }}
      >
        <Icon name="check" />
        <Step.Content>
