        ...
        <Segment style={{ borderTop: '2px solid #663399' }}>
          ...
          {!this.props.uuid && (
            <div style={{ textAlign: 'right' }}>
              <a
                href="/api/api-gateway/ozwillo-portal/my/profile"
                target="_blank"
                className="ui button"
                style={{
                  display: 'inline-flex',
              ...
              >
                <img
                  style={{ height: '1.5em', float: 'left', marginRight: '1em' }}
                  src={process.env.PUBLIC_URL + '/img/logo_ozwillo.png'}
                  alt="Ozwillo"
                />
  ...
  render() {
    ...
    const modulesRows = modules.map(moduleName => (
      <Label
        ...
        color={
          profile.localAuthority.activatedModules.includes(moduleName)
            ? 'green'
            : 'red'
        }
      >
    ...
    const serviceOrganisations = this.state.serviceOrganisationAvailable.map(
      service => {
        return {
          key: service.id,
    ...
    const sesileConnection = profile.localAuthority.activatedModules.map(
      activatedModule =>
        activatedModule === 'PES' &&
        this.state.sesileSubscription &&
        this.state.sesileConfiguration && (
          <div key={activatedModule} style={{ marginTop: '1em' }}>
            <Header as="h3" dividing>
              {t('profile.sesile.title')}
            </Header>
            <Field
              htmlFor="serviceOrganisationNumber"
              label={t('profile.sesile.serviceOrganisationNumber')}
            >
              <Dropdown
                compact
                search
            ...
            <Field
              htmlFor="daysToValidated"
              label={t('profile.sesile.validationLimit')}
            >
              <Input
                id="daysToValidated"
                type="number"
    ...
    const profileNotifications = profile.localAuthority.activatedModules.map(
      activatedModule => (
        <div style={{ marginTop: '2em' }} key={activatedModule}>
          ...
          <Field
            htmlFor={activatedModule}
            label={t(`profile.localAuthorityNotifications`)}
          >
            <Checkbox
              toggle
              id={activatedModule}
              checked={profile.localAuthorityNotifications.includes(
                activatedModule
              )}
              onChange={(e, { id, checked }) =>
                onLocalAuthorityNotificationsChange(profile.uuid, id, checked)
              }
            />
          </Field>
          {allNotifications
            .filter(notification =>
              notification.statusType.startsWith(`${activatedModule}_`)
            )
            .map(notification => {
              const notificationValue = profile.notificationValues.find(
                notif => notif.name === notification.statusType
              );
              return (
                <Field
                  key={`${profile.uuid}-${notification.statusType}`}
                  htmlFor={`${profile.uuid}-${notification.statusType}`}
                ...
                >
                  <Checkbox
                    ...
                    checked={
                      !notification.deactivatable ||
                      (notificationValue
                        ? notificationValue.active
      ...
      <div>
        ...
        <Field htmlFor="email" label={t('agent.email')}>
          <Input
            ...
            style={{ maxWidth: '25em' }}
            value={profile.email || ''}
            placeholder={t('profile.no_email')}
            onChange={(e, { id, value }) => onChange(profile.uuid, id, value)}
          />
        </Field>
        {sesileConnection}
        {profile.localAuthority.activatedModules.length > 0 &&
