create table draft (uuid varchar(255) not null, last_modified timestamp, mode varchar(255), decision date, nature int4, primary key (uuid));
  ...
  begin
    for current_rec in select uuid, draft from acte LOOP
      ...
      if draft_tmp_var = true then
        ...
        insert into draft (uuid, last_modified, mode, decision, nature) values (draft_uuid_var, localtimestamp, "ACTE", null, null);
      end if;
    end LOOP;
    return;
  end;
